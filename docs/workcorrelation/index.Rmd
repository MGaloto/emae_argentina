...
time_serie_plot = function(data, name, type){
...
# 
# 
# ggplotly(corr_cross(dataframe[,colnames(dataframe)[c(1:16, 20)]], 
#                  max_pvalue = 0.05, type = 1, top = 60))

# Definir una paleta de colores personalizada con azul y rojo más fuertes
...
valueboxTwo(descripcion = "Sector de Maxima Aceleracion.", 
...
valueboxTwo(descripcion = "Sector de Maxima Desaceleracion." ,
...
Correlacion Sectores
=====================================

**Analisis de correlacion entre sectores.**
...
```{r}
my_colors <- c("#0000FF", "#FF0000")  
cor_matrix <- cor(dataframe[, setdiff(names(dataframe), c("periodos", "year", "Mes"))])

hchart(cor_matrix, type = "heatmap") %>%
  hc_colorAxis(stops = color_stops(2, my_colors)) %>%
  hc_title(text = "Matriz de correlación entre Sectores") %>%
  hc_chart(plotBorderWidth = 1, plotBorderColor = "#cccccc") %>%
